...
from constants import EOF, contentModelFlags, spaceCharacters
...
class AtheistParseErrorToken(Token):
    tokenType = 'AtheistParseError'
    def __init__(self):
        Token.__init__(self)
...
class HTMLTokenizer(object):
    ...
    * self.states
    ...
    def consumeNumberEntity(self, isHex):
        ...
        If not present self.emitParseError() is invoked.
        """
    ...
    def emitCurrentToken(self):
        ...
        if isinstance(token, StartTagToken):
            ...
            # dict(reversed(self.currentToken.data))
            attrsDict = dict(token.data[::-1])
            ...
            self.parser.processDoctype(token.name, token.data)
        elif isinstance(token, ParseErrorToken):
            self.parser.parseError()
        elif isinstance(token, AtheistParseErrorToken):
    ...
    def emitParseError(self):
        self.parser.parseError()
    
    def emitAtheistParseError(self):
        self.parser.atheistParseError()
    ...
    def beforeDoctypeNameState(self):
        ...
        if data in spaceCharacters:
            ...
            self.currentToken = DoctypeToken()
