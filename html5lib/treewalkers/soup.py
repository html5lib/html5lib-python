...
class TreeWalker(_base.NonRecursiveTreeWalker):
    doctype_regexp = re.compile(
        ur'DOCTYPE\s+(?P<name>[^\s]*)(\s*PUBLIC\s*"(?P<publicId>.*)"\s*"(?P<systemId1>.*)"|\s*SYSTEM\s*"(?P<systemId2>.*)")?')
    def getNodeDetails(self, node):
        if isinstance(node, BeautifulSoup): # Document or DocumentFragment
            ...
            if publicId is not None:
                systemId = m.group(u'systemId1')
            else:
                systemId = m.group(u'systemId2')
            return _base.DOCTYPE, name, publicId or u"", systemId or u""
            ...
            return (_base.ELEMENT, namespaces[u"html"], node.name,
